				...
				<div class="InfoWrap">
					...
					<div class="choose">
						...
						<div class="cartWrap">
							...
							<div class="add">
								<!-- 以前咱们得路由跳转：从A路由跳转到B路由，这里在加入购物车进行路由跳转之前，
...
export default {
	...
	methods: {
		...
		async addShopcar() {
			...
			/*
				当前这里是派发了一个action，也像服务器发请求，判断加入购物车是成功还是失败了，进行相应的操作！
				this.$store.dispatch("addOrUpdateShopCart", {skuId: this.$route.params.skuid,skuNum: this.skuNum})
				上面这行代码说白了：调用仓库中的addOrUpdateShopCart,这个方法加上了async，所以返回的一定是promise
				要么成功，要么失败！
			*/
			...
			try {
				...
				// 下面的这种手段路由跳转以及传递参数可以，但是不推荐
				// this.$router.push({
				// 	name: "addcartsuccess",
				// 	query: { skuInfo: this.skuInfo, skuNum: this.skuNum }
				// })
				// 一些简单的数据skuNum，通过query形式给组件路由传递过去
				// 产品的信息的数据【比较复杂：skuInfo】，通过会话技术（不持久化，会话结束数据在消失）
				// 本地存储|会话存储，一般都是存储字符串
