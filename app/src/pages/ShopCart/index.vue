...
export default {
	...
	methods: {
		...
		handler: throttle(async function (type, disNum, cart) {
			// type :区分这三个元素
			// 目前disNum： + 变化量（1） -变化量（-1） input最终的个数（并不是变化量）
			// cart：哪一个产品【身上有id】
			// 向服务器发送请求，修改数量
			switch (type) {
				case "add":
				...
				case "minus":
					...
					break;
				case "change":
					// 用户输入进来的最终量，非法的（带有汉字,出现负数），带给服务器数字为0
					if (isNaN(disNum) || disNum < 1) {
						disNum = 0;
					} else {
						// 属于正常的情况（小数，取整），带给服务器变化的量 用户输入进来的 - 产品的起始个数
	...
	computed: {
		...
		// 判断底部复选框是否勾选【全部产品都勾选才勾选】
		isAllCheck() {
			// 遍历数组里面原理，只要全部元素isChecked == 1 --》true
			// 只要有一个不是 --》false
