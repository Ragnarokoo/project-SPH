								...
								<li :class="{ active: isOne }" @click="changeOrder('1')">
									<a>
										...
										<span
											...
											class="iconfont"
											:class="{
												'icon-BusinessIcons_DownGraphic': isDesc,
												'icon-up': isAsc
											}"
										></span>
									</a>
...
export default {
	...
	beforeMount() {
		...
		// 在发请求之前，把接口需要传递参数，进行整理（再给服务器发请求之前整理好，服务器就会返回查询的数据）
	...
	methods: {
		// 向服务器发请求来获取search模块数据【根据参数不同返回不同数据展示】
		// 把这次请求封装成为一个函数，当你需要调用的时候调用即可！
		getData() {
		...
		removeCategoryName() {
			...
			// 带给服务器参数说明可有可无了，如果属性值为空的字符串还是会把相应的字段带给服务器
			...
			// 严谨：本意是删除query，如果路径中存在params不应该删除，路由跳转的时候应该带着
	...
	watch: {
		...
		$route(newValue, oldValue) {
			...
			// 再次发起ajax请求
			this.getData();
			// 每一次请求完毕：应该把相应的1、2、3级分类的id置空，让它接受下一次的相应的1、2、3id
			this.searchParams.category1Id = undefined;
			this.searchParams.category2Id = undefined;
			this.searchParams.category3Id = undefined;
		}
